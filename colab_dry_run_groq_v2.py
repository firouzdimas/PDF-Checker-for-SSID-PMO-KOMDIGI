...
# REKOMENDASI UTAMA: Gunakan Google Gemini (Gratis, Limit Tinggi 15 RPM / 1 juta TPM, Sangat Stabil)
...
# Auto-load API Keys dari Secrets Colab jika ada
if IN_COLAB:
    try:
        from google.colab import userdata
        colab_gemini = userdata.get("GEMINI_API_KEY")
...
TARGET_SITES = {
    ...
    27: {"site_id": "AM16208465061220N", "nama": "MTS NEGERI 2 PADANG LAWAS UTARA"},
    13: {"site_id": "AM16576892871217N", "nama": "SD NEGERI 26 SIMANINDO SANGKAL"},
    15: {"site_id": "AM16208465031217N", "nama": "LEMBAGA PEMASYARAKATAN KELAS III PANGURURAN"},
    6: {"site_id": "AM17328708931212N", "nama": "KANTOR KEPALA DESA NALELA"},
    10: {"site_id": "AM17307766951212N", "nama": "KANTOR KEPALA DESA DESA PARIK"},
    17: {"site_id": "AM16566413931203N", "nama": "SMP NEGERI 4 ANGKOLA SANGKUNUR"},
    7: {"site_id": "AM17307766951201N", "nama": "KANTOR CAMAT PASARIBU TOBING"},
    11: {"site_id": "AM16568908881202N", "nama": "SD NEGERI 178209 PEATOLONG"},
    23: {"site_id": "AM16208465041202N", "nama": "MADRASAH ALIYAH NEGERI TAPANULI UTARA"},
    33: {"site_id": "AM16208465091202N", "nama": "MADRASAH TSANAWIYAH NEGERI TAPANULI UTARA"},
}
...
def call_gemini_api(image_base64_list, prompt, api_key, max_retries=5, initial_delay=5):
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            if response.status_code == 429:
                print(f"    [!] Gemini Rate limit (429). Menunggu {delay}s ({attempt}/{max_retries})...")
                time.sleep(delay)
                    ...
                    try:
                        ...
                        if json_match:
                            try:
                                ...
                                print(f"    [❌] Gagal parse JSON hasil regex: {je}")
                        print(f"    [❌] Gagal parse JSON. Respons mentah dari Gemini:\n{text}")
            return None
        except Exception as e:
            if attempt == max_retries:
                print(f"    [❌] Gemini API Error pada percobaan terakhir: {e}")
                if 'response' in locals() and response is not None:
                    print(f"    [❌] Status Code: {response.status_code}")
                    print(f"    [❌] Response Body: {response.text}")
                return None
